...
async function approvedPlan(): Promise<void> {
  ...
  await fs.writeFile(path.join(project, "docs", "x-plan.md"), "# plan");
  const v = await VaultManager.open(vaultPath);
  await v.submitForReview("x", "plan", "docs/x-plan.md", "a@o.c", "A");
  const hash = hashContent(await fs.readFile(path.join(project, "docs", "x-plan.md")));
...
describe("cmdCheck exit codes", () => {
  it("returns 0 when the resolved feature's plan is approved", async () => {
    await approvedPlan();
    const code = await cmdCheck([], { SIGNOFF_FEATURE: "x" }, project);
    expect(code).toBe(0);
  });
  ...
  it("resolves the feature from the branch env when no explicit feature", async () => {
    await approvedPlan();
    const code = await cmdCheck([], { SIGNOFF_BRANCH: "feat/x" }, project);
    expect(code).toBe(0);
  });
});
